...
const filterAnimalsBy = {
  ...
  shots: value =>
    R.filter(
      R.anyPass([() => isNullOrEmpty(value), R.propEq("shots", value ? 1 : 0)])
    ),
  neutered: value =>
    R.filter(
      R.anyPass([
        () => isNullOrEmpty(value),
        R.propEq("neutered", value ? 1 : 0)
      ])
    ),
  declawed: value =>
    R.filter(
  ...
  size: value =>
    R.filter(animal =>
      R.anyPass([
        () => isNullOrEmpty(value),
        R.equals(
          R.toLower(weightToSize(R.prop("weight", animal))),
          R.toLower(value ? value : "")
        )
      ])
    ),
  species: value =>
